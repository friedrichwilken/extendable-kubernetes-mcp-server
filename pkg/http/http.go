...
func Serve(ctx context.Context, mcpServer *localmcp.Server, staticConfig *config.StaticConfig, oidcProvider *oidc.Provider, httpClient *http.Client) error {
	...
	case sig := <-sigChan:
		...
		cancel()
	case <-ctx.Done():
		klog.V(0).Infof("Context canceled, initiating graceful shutdown")
	case err := <-serverErr:
