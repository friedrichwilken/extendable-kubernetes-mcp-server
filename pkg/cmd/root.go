// Package cmd provides the CLI command structure for the extendable Kubernetes MCP server.
// It replicates ALL kubernetes-mcp-server functionality providing a clean foundation for future extensions.
//
// The package creates a Cobra-based CLI that provides exactly the same interface and behavior
// as kubernetes-mcp-server, serving as a starting point for building custom extensions.
package cmd
...
extendable-k8s-mcp --version

# start STDIO server
extendable-k8s-mcp

# start a SSE server on port 8080
extendable-k8s-mcp --port 8080

# start a SSE server on port 8443 with a public HTTPS host of example.com
extendable-k8s-mcp --port 8443 --sse-base-url https://example.com:8443

# start a SSE server on port 8080 with multi-cluster tools disabled
extendable-k8s-mcp --port 8080 --disable-multi-cluster
`))
)
...
type ExtendableMCPServerOptions struct {
	Version              bool
	LogLevel             int
	Port                 string
	SSEBaseUrl           string
...
func NewExtendableMCPServerOptions(streams genericiooptions.IOStreams) *ExtendableMCPServerOptions {
	return &ExtendableMCPServerOptions{
		IOStreams:    streams,
...
func NewExtendableMCPServer(streams genericiooptions.IOStreams) *cobra.Command {
	o := NewExtendableMCPServerOptions(streams)
	cmd := &cobra.Command{
		Use:     "extendable-k8s-mcp [command] [options]",
		Short:   "Extendable Kubernetes Model Context Protocol (MCP) server",
		Long:    long,
		Example: examples,
	...
	// Add all kubernetes-mcp-server flags
...
func (m *ExtendableMCPServerOptions) Complete(cmd *cobra.Command) error {
	if m.ConfigPath != "" {
...
func (m *ExtendableMCPServerOptions) initializeLogging() {
